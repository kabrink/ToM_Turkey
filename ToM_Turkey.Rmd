...
```{r pcor_ToM}
cor.test(ToM.T$ToM5tasks, noofyoungersib, data=ToM.T)
part.cor(ToM5tasks, noofyoungersib, age, data=ToM.T) 
part.cor(ToM5tasks, noofoldersib, age, data=ToM.T) 
part.cor(ToM5tasks, ses, age, data=ToM.T)
part.cor(ToM5tasks, numofoldpeop, age, data=ToM.T) 
```
...
summary(aov(ToM5tasks~gender,data=ToM.T))
aggregate(ToM5tasks~gender, ToM.T, mean)
```
...
```{r regression}
#listwise deletion
ToM.T_red = subset(ToM.T, select=c(ToM5tasks,age,gender,numofoldpeop))
ToM.T_red = na.omit(ToM.T_red)
fit1 <- lm(ToM5tasks ~ age, ToM.T_red)
fit2 <- lm(ToM5tasks ~ age + gender, ToM.T_red)
fit3 <- lm(ToM5tasks ~ age + gender + numofoldpeop, ToM.T_red)
...
```{r npar_test}
knowaccess_num = ifelse(knowaccess=='wrong',0,1)
diversebelief_num = ifelse(diversebelief=='wrong',0,1)
...
t.test(ToM5tasks ~ acqpatt,var.equal=TRUE)
t.test(age ~ acqpatt,var.equal=TRUE)
chisq.test(table(gender, acqpatt), correct = FALSE)
sum(table(gender,acqpatt))

aggregate(numofoldpeop~acqpatt,mean,data=ToM.T)

anova(lm(numofoldpeop ~ age + acqpatt))
anova(lm(noofoldersib ~ age + acqpatt))
anova(lm(noofyoungersib ~ age + acqpatt))
...
#Check that this compares to SPSS
logit <- glm(acpat.dum ~ age + numofoldpeop + ses3, family = binomial(logit))
summary(logit)
exp(confint(logit))
...
#Check that this compares to SPSS
logit <- glm(acpat.dum ~ age + numofoldpeop, family = binomial(logit))
summary(logit)
exp(confint(logit))
exp(coefficients(logit))
with(logit, null.deviance - deviance)
...
```{r guttman}
table(c(acqpatt,diversedesire,knowaccess,diversebelief,explicitfb,realappemotion))

table(acqpatt,diversedesire)
...
mean(acqpatt1$realappemotion=='true',na.rm=TRUE)
...
library(sirt)
gutt = prob.guttman(ToM.T[,c('diversedesire','knowaccess','diversebelief','contentsfb','realappemotion.rescored')])
...
library(mRm)
ToM.scale = ToM.T[,c('diversedesire','knowaccess','diversebelief','contentsfb','realappemotion.rescored')]
mrm.mod = mrm(ToM.scale, cl=2, max.it = 10000, conv.crit = .0001)
summary(mrm.mod)
plot(mrm.mod)
```
...
library(mixRasch)
mr.mod = mixRasch(ToM.scale, steps = 1, max.iter = 1000000, conv.crit = 0.001, model = "RSM", n.c = 2)
summary(mr.mod)
```
...
rt <- raschtree(resp~gender, data = ToM.rt)
plot(rt)
```
